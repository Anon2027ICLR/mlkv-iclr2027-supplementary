...
19 generation stores the paper's tables are computed from, and the
...
uv run python scripts/audit_paper_numbers.py       # 434 checks, 0 mismatches
```
...
- `results/*.db` — the generation stores: one SQLite row per model output, with
  the serving stack recorded per run. There are 19: the seventeen the
