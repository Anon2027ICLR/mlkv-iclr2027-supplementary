#!/bin/bash
# Local puller for e_iclr9 (fourth-review Group B: slack_depth, xinstr,
# if_depth, thsw). After a verified pull: touch SYNCED, then
# POST /pods/$POD/stop (guide §12). Also pulls the chain log.
set -u
POD="${POD:-}"
DEST="${DEST:-$HOME/working_space/research/mlkv/results}"
LOG="${LOG:-$HOME/working_space/research/mlkv/results/pull_iclr9.log}"
...
# slack_depth: te 669 x 4 = 2676; xinstr: 100 x 3 x 2 = 600;
# if_depth: 669 x 2 = 1338; thsw: 100 x 3 x 2 = 600.
STORES="slack_depth:2676 xinstr:600 if_depth:1338 thsw:600"
...
while true; do
  ...
  if [ -n "$IP" ] && [ -n "$PORT" ]; then
    info=$(ssh $ssh_opts -p "$PORT" root@"$IP" '
      if grep -q ALL_ICLR9_CHAIN_DONE /workspace/iclr9_chain.log 2>/dev/null; then
        echo DONE; exit 0
      fi
      python3 - <<PY
import sqlite3, os
...
scp -q -P "$PORT" -o StrictHostKeyChecking=accept-new -o ConnectTimeout=30 \
  ...
  "$DEST/q_percentiles_iclr9.json.part" 2>>"$LOG" || true
if [ -f "$DEST/q_percentiles_iclr9.json.part" ]; then
  mv "$DEST/q_percentiles_iclr9.json.part" "$DEST/q_percentiles_iclr9.json"
fi
scp -q -P "$PORT" -o StrictHostKeyChecking=accept-new -o ConnectTimeout=30 \
  root@"$IP":/workspace/iclr9_chain.log "$DEST/iclr9_chain.log.part" 2>>"$LOG" || true
if [ -f "$DEST/iclr9_chain.log.part" ]; then
  mv "$DEST/iclr9_chain.log.part" "$DEST/iclr9_chain.log"
fi
